...
Optionally add analytics pipeline pieces if you want them. None are required. Follow the [Analytics opt-in guide](/analytics/) to pick queue buffering, daily rollups, retention, and/or Analytics Engine. Validate queue and cron locally first if you want ([Validate locally](/analytics/#validate-locally)). The default deploy works without any of those.
...
10. The hosted MCP server stays off until you enable it from **Access → MCP**. If you connect Claude or ChatGPT, turn off Cloudflare Bot Fight Mode and Block AI Bots on that hostname (Free plan) or skip Super Bot Fight Mode for `/mcp` and `/api/auth/oauth2/*` (Pro). See [MCP](/mcp/).
