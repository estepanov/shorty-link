...
The hosted MCP endpoint is `POST /mcp`. Claude and ChatGPT discover OAuth from `/.well-known/oauth-protected-resource` and authorize at `/api/auth/mcp/*`. Tools honor the authenticated user's role, permissions, and domain/link scopes.
